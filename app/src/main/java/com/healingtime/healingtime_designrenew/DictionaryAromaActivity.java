...
public class DictionaryAromaActivity extends AppCompatActivity {
    ListView list;
    DictionaryListViewAdapter adapter;
    EditText editsearch;
    Integer[] aroma_icon;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.dictionary_aroma_layout);

        
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        
        getSupportActionBar().setTitle("아로마 사전");
        ...
        for (int i = 0; i < aroma_name.length; i++)        {
            aroma_nickname = sp_aroma_setting.getString(aroma_name[i], "NA");
            DictionaryAromaDTO wp = new DictionaryAromaDTO("NA", aroma_icon[i], aroma_name[i], aroma_sub_name[i], aroma_nickname
        ...
        adapter = new DictionaryListViewAdapter(this, arraylist);
        
        list.setAdapter(adapter);

        
        RelativeLayout therapy_dictionary_color = (RelativeLayout)findViewById(R.id.therapy_dictionary_color1);
        therapy_dictionary_color.setOnClickListener(new View.OnClickListener(){
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(getApplicationContext(), DictionaryColorActivity.class);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        getMenuInflater().inflate(R.menu.dictionary_aroma_actionbar, menu);
        SearchView searchView = (SearchView) menu.findItem(R.id.aroma_search).getActionView();
